...
export default async function JournalPage({ searchParams }: { searchParams: Promise<{ type?: string }> }) {
  ...
  return (
    <>
      ...
      {activity.length === 0 ? (
        <EmptyState title={t("Aucune entrée")} hint={t("Avancez l'horloge de démo ou cliquez « Évaluer » pour déclencher des automatisations.")} />
      ) : (
        <Card className="p-2">
          <ol className="relative">
            {activity.map((ev) => (
              <li key={ev.id} className="flex gap-4 px-3 py-3">
                <div className="w-36 shrink-0 pt-0.5 text-xs text-[var(--color-muted)]">{fmtDateTime(ev.occurredAt, langue)}</div>
