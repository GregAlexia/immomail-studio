...
export default async function LeadsPage() {
  const { agency, t, langue } = await pageContext();
  const a = agency.id;
  ...
  return (
    <>
      <PageHeader
        title={t("Boîte de réception → Leads qualifiés")}
        description={t(
      ...
      >
        {pending.length > 0 && (
          <ActionButton action={evaluateNow}>
            <Play size={16} />{" "}
            {pending.length > 1
              ? `${t("Trier les")} ${pending.length} ${t("emails maintenant")}`
              : t("Trier l'email maintenant")}
          </ActionButton>
        )}
      </PageHeader>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div>
          <div className="mb-3 flex items-center gap-2">
            ...
            <h2 className="font-semibold text-[var(--color-ink)]">{t("Avant — emails bruts reçus")}</h2>
                ...
                <Card key={e.id} className={`p-4 ${e.status === "spam" ? "opacity-70" : ""}`}>
                  <div className="mb-1 flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2">
                      <Badge tone={e.source === "spam" ? "red" : "blue"}>{t(SOURCE[e.source] ?? e.source)}</Badge>
                  ...
                  <p className="text-sm text-[var(--color-muted)]">
                    {e.senderName} · {e.senderEmail} · {t("reçu")} {fmtDateTime(e.receivedAt, langue)}
                  </p>
        ...
        <div>
          <div className="mb-3 flex items-center gap-2">
            ...
            <h2 className="font-semibold text-[var(--color-ink)]">{t("Après — leads structurés dans le CRM")}</h2>
          ...
          {leads.length === 0 ? (
            <EmptyState
              title={t("Aucun lead pour l'instant")}
              hint={t(
            ...
            <Card>
              <Table head={<><Th>{t("Lead")}</Th><Th>{t("Demande")}</Th><Th>{t("Routage")}</Th><Th>{t("Réponse auto")}</Th></>}>
                {leads.map((l) => {
                  ...
                  return (
                    <Tr key={l.id}>
                      <Td>
                        ...
                        <p className="text-xs text-[var(--color-muted)]">{l.phone ?? t("Tél. non communiqué")}</p>
                        <span className="text-[10px] font-semibold text-[var(--color-muted)]">{l.externalId}</span>
                      </Td>
                      <Td>
                        <Badge tone="violet">{t(REQ[l.requestType ?? "purchase"])}</Badge>
                        {prop && <p className="mt-1 text-xs text-[var(--color-muted)]">{prop.ref} — {prop.title}</p>}
                        <p className="mt-1"><Badge tone={l.priority === "haute" ? "red" : "gray"}>{t("Priorité")} {t(l.priority ?? "moyenne")}</Badge></p>
                      </Td>
                      <Td><span className="text-sm text-[var(--color-ink)]">{l.assignedTo}</span></Td>
                      <Td>
                        {l.firstResponseAt ? (
                          <Badge tone="green">{t("Envoyée")} {fmtDateTime(l.firstResponseAt, langue)}</Badge>
                        ) : (
                          <Badge tone="amber">{t("En attente")}</Badge>
                        )}
                      </Td>
                    </Tr>
                  );
