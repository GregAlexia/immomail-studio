...
/** Demande de rappel déposée depuis la page de vente. */
export async function actionDemanderRappel(donnees: FormData): Promise<void> {
  ...
  if (texte(donnees.get("site"), 200).length > 0) redirect("/presentation?envoye=1#rappel");

  const nom = texte(donnees.get("nom"), 80);
  const email = texte(donnees.get("email"), 180).toLowerCase();
  if (nom.length === 0) redirect("/presentation?erreur=nom#rappel");
  if (!emailPlausible(email)) redirect("/presentation?erreur=email#rappel");
  ...
  redirect("/presentation?envoye=1#rappel");
}
