 ...
 * (KE-001…302, emails KE-EM-1…8). Azur et Capitale, plus légères, démontrent
...
export async function semerEspace(workspaceId: string): Promise<SeedCounts> {
  ...
  const ke001 = property(A4, { ref: "KE-001", title: "Maison de ville près de la Place Ducale", type: "house", transaction: "sale", price: 168000, surface: 95, rooms: 4, city: "Charleville-Mézières", zone: "Centre-ville", negotiator: "Céline Warnier", status: "available" });
  const ke002 = property(A4, { ref: "KE-002", title: "Maison familiale avec jardin à Warcq", type: "house", transaction: "sale", price: 239000, surface: 135, rooms: 6, city: "Warcq", zone: "Warcq", negotiator: "Damien Poncelet", status: "available" });
  const ke003 = property(A4, { ref: "KE-003", title: "T3 rénové quartier Manchester", type: "apartment", transaction: "sale", price: 118000, surface: 68, rooms: 3, city: "Charleville-Mézières", zone: "Manchester", negotiator: "Nathalie Thiry", status: "under_offer" });
  property(A4, { ref: "KE-004", title: "T2 bords de Meuse", type: "apartment", transaction: "sale", price: 84000, surface: 48, rooms: 2, city: "Charleville-Mézières", zone: "Centre-ville", negotiator: "Nathalie Thiry", status: "available" });
  const ke005 = property(A4, { ref: "KE-005", title: "Maison de maître à Mohon", type: "house", transaction: "sale", price: 275000, surface: 180, rooms: 8, city: "Charleville-Mézières", zone: "Mohon", negotiator: "Damien Poncelet", status: "available" });
  property(A4, { ref: "KE-006", title: "T3 centre-ville (location)", type: "apartment", transaction: "rental", price: 620, surface: 70, rooms: 3, city: "Charleville-Mézières", zone: "Centre-ville", negotiator: "Nathalie Thiry", status: "available" });
  const ke007 = property(A4, { ref: "KE-007", title: "Local commercial Place Ducale", type: "commercial", transaction: "sale", price: 189000, surface: 75, rooms: 0, city: "Charleville-Mézières", zone: "Centre-ville", negotiator: "Céline Warnier", status: "available" });
  const ke101 = property(A4, { ref: "KE-101", title: "T2 loué Ronde Couture", type: "apartment", transaction: "rental", price: 480, surface: 52, rooms: 2, city: "Charleville-Mézières", zone: "Ronde Couture", negotiator: "Nathalie Thiry", status: "rented" });
  const ke102 = property(A4, { ref: "KE-102", title: "Studio loué centre-ville", type: "studio", transaction: "rental", price: 355, surface: 26, rooms: 1, city: "Charleville-Mézières", zone: "Centre-ville", negotiator: "Céline Warnier", status: "rented" });
  const ke103 = property(A4, { ref: "KE-103", title: "Maison louée à Villers-Semeuse", type: "house", transaction: "rental", price: 780, surface: 105, rooms: 5, city: "Villers-Semeuse", zone: "Villers-Semeuse", negotiator: "Damien Poncelet", status: "rented" });
  const ke202 = property(A4, { ref: "KE-202", title: "T3 vendu Manchester", type: "apartment", transaction: "sale", price: 112000, surface: 65, rooms: 3, city: "Charleville-Mézières", zone: "Manchester", negotiator: "Nathalie Thiry", status: "sold" });
  const ke203 = property(A4, { ref: "KE-203", title: "Maison vendue à Nouzonville", type: "house", transaction: "sale", price: 156000, surface: 110, rooms: 5, city: "Nouzonville", zone: "Nouzonville", negotiator: "Damien Poncelet", status: "sold" });
  property(A4, { ref: "KE-301", title: "T4 centre-ville", type: "apartment", transaction: "sale", price: 145000, surface: 88, rooms: 4, city: "Charleville-Mézières", zone: "Centre-ville", negotiator: "Céline Warnier", status: "available" });
  property(A4, { ref: "KE-302", title: "T2 Mohon (location)", type: "apartment", transaction: "rental", price: 495, surface: 44, rooms: 2, city: "Charleville-Mézières", zone: "Mohon", negotiator: "Nathalie Thiry", status: "available" });

  const keoDidier = contact(A4, "Didier", "Lallemand", "owner", { email: "d.lallemand@orange.fr", phone: "06 24 51 63 07" });
  const keoMartine = contact(A4, "Martine", "Piérard", "owner", { email: "martine.pierard@gmail.com", phone: "06 35 72 18 09" });
  const keoBernard = contact(A4, "Bernard", "Colin", "owner", { email: "bernard.colin@wanadoo.fr", phone: "06 46 83 29 10" });
  const keoFatima = contact(A4, "Fatima", "Zerrouki", "owner", { email: "fatima.zerrouki@gmail.com", phone: "06 57 94 30 21" });
  // Le mandat de KE-002 expire à J+6 : A3 se déclenche dès les premiers jours.
  M.push(
    { id: uid(), agencyId: A4, propertyId: ke001, ownerId: keoDidier, type: "exclusive", startDate: dDay(-320), endDate: dDay(18), status: "active", createdAt },
    { id: uid(), agencyId: A4, propertyId: ke002, ownerId: keoMartine, type: "exclusive", startDate: dDay(-280), endDate: dDay(6), status: "active", createdAt },
    { id: uid(), agencyId: A4, propertyId: ke005, ownerId: keoBernard, type: "simple", startDate: dDay(-180), endDate: dDay(52), status: "active", createdAt },
    { id: uid(), agencyId: A4, propertyId: ke007, ownerId: keoFatima, type: "exclusive", startDate: dDay(-90), endDate: dDay(70), status: "active", createdAt }
  );

  const ketSamir = contact(A4, "Samir", "Bouchard", "tenant", { email: "samir.bouchard@gmail.com", phone: "06 68 15 42 33" });
  const ketAurelie = contact(A4, "Aurélie", "Deville", "tenant", { email: "aurelie.deville@free.fr", phone: "06 79 26 53 44" });
  const ketKevin = contact(A4, "Kévin", "Gérard", "tenant", { email: "kevin.gerard@laposte.net", phone: "06 80 37 64 55" });
  ...
  L.push(
    { id: uid(), agencyId: A4, propertyId: ke101, tenantId: ketSamir, monthlyRent: 480, charges: 35, startDate: dDay(-380), endDate: dDay(350), rentDueDay: 3, createdAt },
    { id: uid(), agencyId: A4, propertyId: ke102, tenantId: ketAurelie, monthlyRent: 355, charges: 25, startDate: dDay(-210), endDate: dDay(155), rentDueDay: 8, createdAt },
    { id: uid(), agencyId: A4, propertyId: ke103, tenantId: ketKevin, monthlyRent: 780, charges: 90, startDate: dDay(-450), endDate: dDay(280), rentDueDay: 15, createdAt }
  );

  CI.push(
    { id: uid(), agencyId: A4, propertyId: ke005, type: "dpe", label: "DPE à renouveler", dueDate: dDay(9), reminderDaysBefore: 15, status: "pending", createdAt },
    { id: uid(), agencyId: A4, propertyId: ke001, type: "dpe", label: "DPE à renouveler", dueDate: dDay(16), reminderDaysBefore: 30, status: "pending", createdAt },
    { id: uid(), agencyId: A4, propertyId: ke103, type: "pno_insurance", label: "Assurance PNO à renouveler", dueDate: dDay(35), reminderDaysBefore: 30, status: "pending", createdAt },
    { id: uid(), agencyId: A4, propertyId: ke101, type: "lease_renewal", label: "Renouvellement de bail", dueDate: dDay(44), reminderDaysBefore: 15, status: "pending", createdAt }
  );

  // Signatures échelonnées comme chez Horizon : J-2 (demande d'avis à venir),
  // J-6 (relance A7 due) et J-33 (parrainage A8 dû).
  const kecLaurent = contact(A4, "Laurent", "Mangin", "buyer", { email: "laurent.mangin@gmail.com", phone: "06 91 48 75 66" });
  const kecSophie = contact(A4, "Sophie", "Renard", "buyer", { email: "sophie.renard@orange.fr", phone: "06 02 59 86 77" });
  const kecYoussef = contact(A4, "Youssef", "Amrani", "buyer", { email: "youssef.amrani@gmail.com", phone: "06 13 60 97 88" });
  TX.push(
    { id: uid(), agencyId: A4, propertyId: ke003, contactId: kecLaurent, type: "sale", signedDate: dDay(-2), reviewRequestedAt: null, reviewFollowupAt: null, reviewCompletedAt: null, referralRequestedAt: null, createdAt },
    { id: uid(), agencyId: A4, propertyId: ke202, contactId: kecSophie, type: "sale", signedDate: dDay(-6), reviewRequestedAt: null, reviewFollowupAt: null, reviewCompletedAt: null, referralRequestedAt: null, createdAt },
    { id: uid(), agencyId: A4, propertyId: ke203, contactId: kecYoussef, type: "sale", signedDate: dDay(-33), reviewRequestedAt: null, reviewFollowupAt: null, reviewCompletedAt: null, referralRequestedAt: null, createdAt }
  );
  ...
  AP.push(
    { id: uid(), agencyId: A4, propertyId: ke001, contactId: null, contactName: "Vincent Maréchal", contactEmail: "v.marechal@gmail.com", contactPhone: "06 44 90 12 34", type: "visite", scheduledAt: at(1, 17, 30), status: "confirmed", confirmationSentAt: null, reminderJ1SentAt: null, reminderH2SentAt: null, createdAt },
    { id: uid(), agencyId: A4, propertyId: ke002, contactId: null, contactName: "Sandrine Leroy", contactEmail: "sandrine.leroy@orange.fr", contactPhone: "06 55 01 23 45", type: "visite", scheduledAt: at(2, 10, 30), status: "confirmed", confirmationSentAt: null, reminderJ1SentAt: null, reminderH2SentAt: null, createdAt },
    ...
    { id: uid(), agencyId: A4, propertyId: ke005, contactId: null, contactName: "Amandine Roussel", contactEmail: "amandine.roussel@gmail.com", contactPhone: "06 66 12 34 56", type: "visite", scheduledAt: at(6, 9, 30), status: "confirmed", confirmationSentAt: null, reminderJ1SentAt: null, reminderH2SentAt: null, createdAt }
  );

  email("KE-EM-1", "seloger", "Vincent Maréchal", "v.marechal@gmail.com", "Maison centre-ville Charleville", "Bonjour, votre maison de ville près de la Place Ducale à 168 000 € m'intéresse. Serait-il possible de la visiter en fin de semaine ? Cordialement", at(-1, 8, 41), "KE-001", A4);
  email("KE-EM-2", "leboncoin", "Sandrine Leroy", "sandrine.leroy@orange.fr", "Maison Warcq avec jardin", "Bonjour, la maison de Warcq est-elle toujours en vente ? Nous sommes une famille de 5 et le jardin nous intéresse beaucoup. 06 55 01 23 45", at(-1, 10, 27), "KE-002", A4);
  email("KE-EM-3", "site", "Patrick Guillaume", "p.guillaume@wanadoo.fr", "Estimation maison Aiglemont", "Bonjour, je souhaite faire estimer ma maison de 120 m² à Aiglemont avant une mise en vente au printemps. Quelles sont vos disponibilités ?", at(-1, 13, 15), null, A4);
  email("KE-EM-4", "bienici", "Julie Wathelet", "julie.wathelet@gmail.com", "Location T3 centre-ville", "Bonjour, le T3 à 620 €/mois en centre-ville est-il encore libre ? Je suis infirmière au CH de Charleville, CDI. Merci d'avance", at(-1, 15, 52), "KE-006", A4);
  email("KE-EM-5", "autre", "Amandine Roussel", "amandine.roussel@gmail.com", "Maison de maître Mohon", "Bonjour, nous aimerions visiter la maison de maître de Mohon. Nous sommes disponibles en semaine après 17 h. 06 66 12 34 56", at(0, 9, 8), "KE-005", A4);
  email("KE-EM-6", "autre", "Karim Belaïd", "karim.belaid@gmail.com", "Pièces dossier location T3", "Bonjour, je vous joins mes trois derniers bulletins de salaire et mon avis d'imposition pour le dossier de location. Dites-moi s'il manque quelque chose.", at(0, 11, 33), "KE-006", A4);
  email("KE-EM-7", "seloger", "Damien Charpentier", "d.charpentier@orange.fr", "Local commercial Place Ducale", "Bonjour, je cherche un local pour ouvrir un salon de thé. Les 75 m² de la Place Ducale conviendraient. Quelle est la surface de vitrine ?", at(0, 14, 6), "KE-007", A4);
